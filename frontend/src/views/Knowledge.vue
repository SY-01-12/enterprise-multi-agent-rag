...
function toggleExpand(kb) {
  const idx = expandedRows.value.indexOf(kb.id)
  if (idx >= 0) {
    expandedRows.value.splice(idx, 1)
  } else {
    expandedRows.value.push(kb.id)
    loadDocuments(kb)
  }
}
...
<template>
  <div class="kb-page">
    ...
    <div class="upload-area">
      ...
      <el-upload ref="uploadRef" :auto-upload="false" :limit="1" accept=".pdf,.docx,.txt"
    ...
    <p class="hint">支持 PDF / DOCX / TXT，最大 50MB。上传后自动解析+向量化+全文索引。</p>
